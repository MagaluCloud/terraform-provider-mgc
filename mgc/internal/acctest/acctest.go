// Package acctest provides the shared harness for terraform-plugin-testing
// acceptance tests. Tests are endpoint-agnostic: the API they talk to (a fake
// implementation or the real cloud) is decided solely by MGC_ENDPOINT, and the
// tests must behave identically against either.
package acctest
...
func PreCheck(t *testing.T) {
	...
	if Endpoint() == "" {
		t.Fatalf("%s must be set to the root URL of the API under test (fake or production)", EnvEndpoint)
	}
	if APIKey() == "" {
...
`, APIKey(), Region(), endpoints.String())
}
