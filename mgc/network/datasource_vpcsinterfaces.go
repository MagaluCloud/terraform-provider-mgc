...
type NetworkVPCInterfacesModel struct {
	Items []NetworkVPCInterfaceDataSourceModel `tfsdk:"items"`
}
...
func (r *NetworkVPCInterfacesDatasource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	for _, vpcInterface := range vpcInterfaces {
		data.Items = append(data.Items, toNetworkVPCInterfaceDataSourceModel(vpcInterface))
	}

	resp.Diagnostics.Append(resp.State.Set(ctx, data)...)
}

func toNetworkVPCInterfaceDataSourceModel(vpcInterface netSDK.PortResponse) NetworkVPCInterfaceDataSourceModel {
